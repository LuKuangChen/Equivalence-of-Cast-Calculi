module illustration.LazyDHypercoercions
  (Label : Set)
  where
...
g-dom : ∀ {T1 T2 T3 T4}
  ...
  → GapT T3 T1
g-dom none = none
g-dom (some l (` T1 ⇒ T2) (` T3 ⇒ T4)) = some (neg l) T3 T1
...
lem-seq-m (` (c2 ⇒̂ d2)) (‼ (S1 ⇒ T1)) (⁇ (S2 ⇒ T2) l) (` (c3 ⇒̂ d3))
          (lam⟨  c1 ⇒ d1 ⟩ e E) | yes ⌣⇒
  rewrite sym (seq-assoc c3 none ⌈ S2 ⟹[ neg l ] S1 ⌉ none (c2 ⨟ c1))
...
correctness-2 : ∀ {T e}
  ...
  → Evalₛ H e o
correctness-2
  = theorem-LazyD-CastADT-correct-part-2 H HIsLazyD
